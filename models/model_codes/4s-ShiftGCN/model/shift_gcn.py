...
class Shift_gcn(nn.Module):
    def __init__(self, in_channels, out_channels, A, coff_embedding=4, num_subset=3):
        ...
        self.Feature_Mask = nn.Parameter(torch.ones(1,67,in_channels, requires_grad=True,device='cuda'),requires_grad=True)
        nn.init.constant(self.Feature_Mask, 0)

        self.bn = nn.BatchNorm1d(67*out_channels)
        self.relu = nn.ReLU()
        ...
        index_array = np.empty(67*in_channels).astype(np.int)
        for i in range(67):
            for j in range(in_channels):
                index_array[i*in_channels + j] = (i*in_channels + j + j*in_channels)%(in_channels*67)
        self.shift_in = nn.Parameter(torch.from_numpy(index_array),requires_grad=False)

        index_array = np.empty(67*out_channels).astype(np.int)
        for i in range(67):
            for j in range(out_channels):
                index_array[i*out_channels + j] = (i*out_channels + j - j*out_channels)%(out_channels*67)
    ...
    def forward(self, x0):
        ...
        x = x.contiguous() + self.down(x0.contiguous()).contiguous()
        x = self.relu(x)
        return x
...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=67, num_person=2, graph=None, graph_args=dict(), in_channels=3):
