...
writer = SummaryWriter("/home/neel1998/runs_resgcn_ntu60_smpl_joint_67_xview_norm2/")
loss_writer = SummaryWriter("/home/neel1998/runs_resgcn_ntu60_smpl_joint_67_xview_norm2/")
...
train_logger = setup_logger('Training_accuracy', '/home/neel1998/runs_resgcn_ntu60_smpl_joint_67_xview_norm2/train_output.log')

# logger for evaluation accuracies
eval_logger = setup_logger('Evaluation_accuracy', '/home/neel1998/runs_resgcn_ntu60_smpl_joint_67_xview_norm2/eval_output.log')
...
class Processor(Initializer):
    ...
    def train(self, epoch):
        ...
        for num, (x, y, name) in enumerate(train_iter):
            ...
            if self.no_progress_bar:
                logging.info('Epoch: {}/{}, Batch: {}/{}, Loss: {:.4f}, LR: {:.9f}'.format(
                    epoch+1, self.max_epoch, num+1, len(self.train_loader), loss.item(), lr
                ))
            else:
                train_iter.set_description('Loss: {:.4f}, LR: {:.9f}'.format(loss.item(), lr))
        # Saving the model
        state_dict = self.model.module.state_dict()
        weights = OrderedDict([[k.split('module.')[-1],
                                 v.cpu()] for k, v in state_dict.items()])

        model_name = '{}_{}_{}'.format(self.args.config, self.args.model_type, self.args.dataset)
        torch.save(weights, "saved_models/" + model_name + '-' + str(epoch) + '.pt')
    ...
    def eval(self, epoch):
        ...
        # Flatten lists to store corresponding names and accuracies
        flat_list = []
        for sublist in names:
           for item in sublist:
             flat_list.append(item)
 
        score_dict = dict(zip(flat_list, score))
        ...
        model_name = '{}_{}_{}'.format(self.args.config, self.args.model_type, self.args.dataset) 
        with open('./workdir//eval_results/' + model_name + '_epoch_' + str(epoch) + '_' + str(avg_acc.item()) + '.pkl'.format(
                epoch, avg_acc), 'wb') as f:
