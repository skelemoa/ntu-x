...
class unit_tcn(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=9, stride=1, num_point=67, block_size=41):
...
class unit_gcn(nn.Module):
    def __init__(self, in_channels, out_channels, A, groups, num_point, coff_embedding=4, num_subset=3):
        ...
        self.eyes = nn.Parameter(torch.tensor(torch.stack(
            eye_array), requires_grad=False, device='cuda'), requires_grad=False)  # [c,67,67]
...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=67, num_person=2, groups=8, block_size=41, graph=None, graph_args=dict(), in_channels=3):
