...
train_data_bone_files = sorted(os.listdir("/ssd_scratch/cvit/nateshhariharan/DecoupleGCN_67_joints_ntu60_bone/data/ntu60_smpl/xsub/bone_data/h5_data_118/train/"))
test_data_bone_files = sorted(os.listdir("/ssd_scratch/cvit/nateshhariharan/DecoupleGCN_67_joints_ntu60_bone/data/ntu60_smpl/xsub/bone_data/h5_data_118/test/"))

train_data_bone_path = "/ssd_scratch/cvit/nateshhariharan/DecoupleGCN_67_joints_ntu60_bone/data/ntu60_smpl/xsub/bone_data/h5_data_118/train/"
test_data_bone_path = "/ssd_scratch/cvit/nateshhariharan/DecoupleGCN_67_joints_ntu60_bone/data/ntu60_smpl/xsub/bone_data/h5_data_118/test/"
...
class Feeder(Dataset):
    ...
    def load_data(self):
        ...
        if self.use_mmap:
            if "train" in self.data_path:
                self.data = np.zeros(len(train_data_bone_files))
            else:
                self.data = np.zeros(len(test_data_bone_files))
        else:
            if "train" in self.data_path:
                self.data = np.zeros(len(train_data_bone_files))
            else:
    ...
    def __getitem__(self, index):
        ...
        if "train" in self.data_path and "bone" in self.data_path:
           ...
           f = h5py.File(train_data_bone_path + train_data_file, "r")
           ...
           f = h5py.File(test_data_bone_path + test_data_file, "r")
           data_numpy = f[list(f.keys())[-1]][:]
           label = int(filename[filename.index("A") + 1: filename.index("A") + 4]) - 1
           f.close()
